...
var GameServer = function() {
  ...
  self.setupVariables = function() {
    ...
    self.domain    = process.env.OPENSHIFT_APP_DNS || '127.0.0.1';
    // Use websocket port 8000 in OpenShift, otherwise port 8080
    self.clientport = (process.env.OPENSHIFT_NODEJS_IP) ? 8000:8080;
  };
  ...
  self.init = function() {
    ...
    self.app.use('/websocketURI.js', function(req, res) {
      var port = self.clientport;
      console.log("domain = ", self.domain, "port:", port);
        // Modify the URI only if we pass an optional connection port in.
        var websocketURI = self.domain + ':' + self.clientport;
